...
static int cf_check silo_evtchn_unbound(struct domain *d1, struct evtchn *chn,
                                        domid_t id2)
{
    int rc = -EPERM;
...
static int cf_check silo_evtchn_interdomain(struct domain *d1,
                                            struct evtchn *chan1,
                                            struct domain *d2,
                                            struct evtchn *chan2)
{
...
static int cf_check silo_grant_mapref(struct domain *d1, struct domain *d2,
                                      uint32_t flags)
{
    if ( silo_mode_dom_check(d1, d2) )
...
static int cf_check silo_argo_register_single_source(const struct domain *d1,
...
static int cf_check silo_argo_send(const struct domain *d1,
                                   const struct domain *d2)
{
    if ( silo_mode_dom_check(d1, d2) )
