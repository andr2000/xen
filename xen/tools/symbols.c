...
static void usage(void)
{
    fprintf(
        stderr,
        "Usage: symbols [--all-symbols] [--symbol-prefix=<prefix char>] < in.map > out.S\n");
    exit(1);
}
...
static int read_symbol(FILE *in, struct sym_entry *s)
{
    ...
    case fmt_sysv:
        while ( fscanf(in, "\n") == 1 )
            /* nothing */;
        rc = fscanf(in, "%499[^ |] |%llx | %c |", str, &s->addr, &stype);
    ...
    if ( strcasecmp(type, "FILE") == 0 ||
	      ...
	      */
          input_format == fmt_sysv && !*type && stype == '?' && sym && sym[1] &&
          strchr("cSsoh", sym[1]) && !sym[2]) )
    {
        free(filename);
    ...
    else if ( toupper((uint8_t)stype) == 'A' )
    {
        ...
        if ( strcmp(sym, "__gp") )
            goto skip_tail;
              ...
              toupper((uint8_t)stype) == 'N' || is_arm_mapping_symbol(sym) )
        goto skip_tail;
...
static int symbol_valid(struct sym_entry *s)
{
    ...
    if ( !all_symbols )
    {
        ...
        if ( (s->addr == _etext && strcmp((char *)s->sym + offset, "_etext")) ||
             ...
             (s->addr == _eextratext &&
              strcmp((char *)s->sym + offset, "_eextratext")) )
            return 0;
    }
...
static void write_src(void)
{
	 ...
	 * every 256 symbols */
    markers = (unsigned int *)malloc(sizeof(unsigned int) *
                                     ((table_cnt + 255) / 256));
...
{
    unsigned int i;
    bool unsorted = false, warn_dup = false, error_dup = false,
         found_dup = false;
    ...
    if ( warn_dup )
    {
        ...
        for ( i = 1; i < table_cnt; ++i )
            if ( strcmp(SYMBOL_NAME(table + i - 1), SYMBOL_NAME(table + i)) ==
                     0 &&
