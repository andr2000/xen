    ...
    for ( prop = sym->prop; prop; prop = prop->next )
    {
        ...
        case P_DEFAULT:
            if ( (sym->type == S_STRING || sym->type == S_INT ||
                ...
                prop_warn(
                    prop,
                    "default for config symbol '%s'" " must be a single symbol",
                    sym->name);
        ...
        case P_IMPLY:
            ...
            if ( sym->type != S_BOOLEAN && sym->type != S_TRISTATE )
                prop_warn(
                    prop,
                    "config symbol '%s' uses %s, but is " "not bool or tristate",
                    sym->name,
                    use);
                ...
                prop_warn(
                    prop,
                    "'%s' has wrong type. '%s' only " "accept arguments of bool and " "tristate type",
                    sym2->name,
                    use);
            break;
        case P_RANGE:
                           ...
                           struct list_head *head)
{
    ...
    if ( head && location )
    {
        ...
        if ( list_empty(head) )
            ...
            jump->index =
                list_entry(head->prev, struct jump_key, entries)->index + 1;
                           ...
                           struct list_head *head)
{
    ...
    if ( sym->rev_dep.expr )
    {
        expr_gstr_print_revdep(sym->rev_dep.expr,
                               r,
                               yes,
                               "  Selected by [y]:\n");
        expr_gstr_print_revdep(sym->rev_dep.expr,
