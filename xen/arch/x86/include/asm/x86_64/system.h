...
static always_inline __uint128_t __cmpxchg16b(volatile void *ptr,
                                              const __uint128_t *oldp,
                                              const __uint128_t *newp)
{
    union {
        struct {
            uint64_t lo, hi;
    ...
    asm volatile(
        "lock cmpxchg16b %[ptr]"
        : "=d"(prev.hi), "=a"(prev.lo), [ptr] "+m"(*(volatile __uint128_t *)ptr)
        : "c"(new.hi), "b"(new.lo), "d"(old.hi), "a"(old.lo));

    return prev.raw;
}

static always_inline __uint128_t cmpxchg16b_local_(void *ptr,
                                                   const __uint128_t *oldp,
                                                   const __uint128_t *newp)
{
    union {
    ...
    asm volatile("cmpxchg16b %[ptr]"
                 : "=d"(prev.hi), "=a"(prev.lo), [ptr] "+m"(*(__uint128_t *)ptr)
                 : "c"(new.hi), "b"(new.lo), "d"(old.hi), "a"(old.lo));
