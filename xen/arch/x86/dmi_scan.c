...
void __init dmi_efi_get_table(const void *smbios, const void *smbios3)
{
    ...
    if ( eps3 && memcmp(eps3->anchor, "_SM3_", 5) == 0 &&
         eps3->length >= sizeof(*eps3) && dmi_checksum(eps3, eps3->length) )
    {
    ...
    if ( eps && memcmp(eps->anchor, "_SM_", 4) == 0 &&
         eps->length >= sizeof(*eps) && dmi_checksum(eps, eps->length) )
    {
...
const char *__init dmi_get_table(paddr_t *base, u32 *len)
{
    ...
    else
    {
        ...
        for ( q = p; q <= p + 0x10000 - sizeof(eps.dmi); q += 16 )
        {
            ...
            if ( !(instance & 1) && memcmp(eps.dmi.anchor, "_DMI_", 5) == 0 &&
                 dmi_checksum(&eps.dmi, sizeof(eps.dmi)) )
            {
            ...
            if ( (instance & 2) || q > p + 0x10000 - sizeof(eps.smbios3) )
                continue;
            memcpy_fromio(&eps.dmi + 1,
                          q + sizeof(eps.dmi),
...
static int __init dmi_iterate(void (*decode)(const struct dmi_header *))
{
    ...
    for ( q = p; q < p + 0x10000; q += 16 )
    {
        ...
        if ( !smbios3.length && q <= p + 0x10000 - sizeof(smbios3) )
        {
...
static int __init dmi_efi_iterate(void (*decode)(const struct dmi_header *))
{
    ...
    while ( efi.smbios3 != EFI_INVALID_TABLE_ADDR )
    {
        ...
        if ( memcmp(eps.anchor, "_SM3_", 5) || eps.length < sizeof(eps) )
            break;
    ...
    if ( ret != 0 && efi.smbios != EFI_INVALID_TABLE_ADDR )
    {
        ...
        if ( memcmp(eps.anchor, "_SM_", 4) || eps.length < sizeof(eps) )
            return -1;
...
static const struct dmi_blacklist __initconstrel dmi_blacklist[] = {
    ...
    { dmi_disable_acpi,
     "IBM Thinkpad", { MATCH(DMI_BOARD_VENDOR, "IBM"),
    ...
    { force_acpi_ht,
     "FSC Primergy T850", { MATCH(DMI_SYS_VENDOR, "FUJITSU SIEMENS"),
        MATCH(DMI_PRODUCT_NAME, "PRIMERGY T850"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "DELL GX240", { MATCH(DMI_BOARD_VENDOR, "Dell Computer Corporation"),
        MATCH(DMI_BOARD_NAME, "OptiPlex GX240"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "HP VISUALIZE NT Workstation", { MATCH(DMI_BOARD_VENDOR, "Hewlett-Packard"),
        MATCH(DMI_PRODUCT_NAME, "HP VISUALIZE NT Workstation"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "Compaq Workstation W8000", { MATCH(DMI_SYS_VENDOR, "Compaq"),
        MATCH(DMI_PRODUCT_NAME, "Workstation W8000"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "ASUS P4B266", { MATCH(DMI_BOARD_VENDOR, "ASUSTeK Computer INC."),
        MATCH(DMI_BOARD_NAME, "P4B266"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "ASUS P2B-DS", { MATCH(DMI_BOARD_VENDOR, "ASUSTeK Computer INC."),
        MATCH(DMI_BOARD_NAME, "P2B-DS"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "ASUS CUR-DLS", { MATCH(DMI_BOARD_VENDOR, "ASUSTeK Computer INC."),
        MATCH(DMI_BOARD_NAME, "CUR-DLS"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "ABIT i440BX-W83977", { MATCH(DMI_BOARD_VENDOR, "ABIT <http://www.abit.com>"),
        MATCH(DMI_BOARD_NAME, "i440BX-W83977 (BP6)"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "IBM Bladecenter", { MATCH(DMI_BOARD_VENDOR, "IBM"),
        MATCH(DMI_BOARD_NAME, "IBM eServer BladeCenter HS20"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "IBM eServer xSeries 360", { MATCH(DMI_BOARD_VENDOR, "IBM"),
        MATCH(DMI_BOARD_NAME, "eServer xSeries 360"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "IBM eserver xSeries 330", { MATCH(DMI_BOARD_VENDOR, "IBM"),
        MATCH(DMI_BOARD_NAME, "eserver xSeries 330"),
        NO_MATCH,
        NO_MATCH } },

    { force_acpi_ht,
     "IBM eserver xSeries 440", { MATCH(DMI_BOARD_VENDOR, "IBM"),
