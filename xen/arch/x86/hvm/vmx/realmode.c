...
static void realmode_deliver_exception(unsigned int vector,
...
again:
    ...
    {
        hvmemul_ctxt->intr_shadow &= ~(VMX_INTR_SHADOW_STI |
...
void vmx_realmode_emulate_one(struct hvm_emulate_ctxt *hvmemul_ctxt)
{
    ...
    if ( rc == X86EMUL_EXCEPTION )
    {
        ...
        {
            gdprintk(XENLOG_ERR,
                     "Exception %02x in protected mode.\n",
                     hvmemul_ctxt->ctxt.event.vector);
            goto fail;
        }
        else
        {
            realmode_deliver_exception(hvmemul_ctxt->ctxt.event.vector,
...
void vmx_realmode(struct cpu_user_regs *regs)
{
    ...
    if ( !curr->arch.hvm.vmx.vmx_emulate && !curr->arch.hvm.vmx.vmx_realmode )
    {
        ...
        sreg->dpl = sreg->sel & 3;
        hvmemul_ctxt.seg_reg_dirty |= (1ul << x86_seg_ds) |
                                      (1ul << x86_seg_es) |
