...
#define WT X86_MT_WT
    ...
    /* MTRR(WC) */
     { UC, WC, RS, RS, UC, UC, WC, WC },
    /* MTRR(RS) */
     { RS, RS, RS, RS, RS, RS, RS, RS },
    /* MTRR(RS) */
     { RS, RS, RS, RS, RS, RS, RS, RS },
    /* MTRR(WT) */
     { UC, WC, RS, RS, WT, WP, WT, UC },
    /* MTRR(WP) */
     { UC, WC, RS, RS, WT, WP, WP, WC },
    /* MTRR(WB) */
     { UC, WC, RS, RS, WT, WP, WB, UC }
...
static uint8_t __read_mostly mtrr_epat_tbl[MTRR_NUM_TYPES][MEMORY_NUM_TYPES] = {
    [0 ... MTRR_NUM_TYPES -
     1] = { [0 ... MEMORY_NUM_TYPES - 1] = INVALID_MEM_TYPE }
};
...
int hvm_vcpu_cacheattr_init(struct vcpu *v)
{
    ...
    unsigned int num_var_ranges = is_hardware_domain(v->domain)
                                      ? MASK_EXTR(mtrr_state.mtrr_cap,
                                                  MTRRcap_VCNT)
                                      : MTRR_VCNT;
    ...
    v->arch.hvm.pat_cr = ((uint64_t)X86_MT_WB) | /* PAT0: WB */
...
int mtrr_get_type(const struct mtrr_state *m, paddr_t pa, unsigned int order)
{
    ...
    if ( overlap_mtrr == ((1 << X86_MT_WT) | (1 << X86_MT_WB)) )
        return X86_MT_WT;
 ...
 */
static uint8_t effective_mm_type(struct mtrr_state *m, uint64_t pat,
                                 paddr_t gpa, uint32_t pte_flags,
                                 uint8_t gmtrr_mtype)
{
...
uint32_t get_pat_flags(struct vcpu *v, uint32_t gl1e_flags, paddr_t gpaddr,
                       paddr_t spaddr, uint8_t gmtrr_mtype)
{
     ...
     */
    guest_eff_mm_type =
        effective_mm_type(g, pat, gpaddr, gl1e_flags, gmtrr_mtype);
    ...
    if ( pat_entry_value == INVALID_MEM_TYPE )
    {
        ...
        if ( p2m_is_ram(p2mt) )
            gdprintk(
                XENLOG_WARNING,
                "Conflict occurs for a given guest l1e flags:%x " "at %" PRIx64
                " (the effective mm type:%d), " "because the host mtrr type is:%d\n",
                gl1e_flags,
                (uint64_t)gpaddr,
                guest_eff_mm_type,
                shadow_mtrr_type);
...
bool mtrr_var_range_msr_set(struct domain *d, struct mtrr_state *m,
...
bool mtrr_pat_not_equal(const struct vcpu *vd, const struct vcpu *vs)
{
    ...
    if ( md->enabled )
    {
        ...
        if ( md->fixed_enabled && memcmp(md->fixed_ranges,
                                         ms->fixed_ranges,
...
void hvm_destroy_cacheattr_region_list(struct domain *d)
{
    ...
    {
        range =
            list_entry(head->next, struct hvm_mem_pinned_cacheattr_range, list);
        list_del(&range->list);
        xfree(range);
    }
}
                                 ...
                                 unsigned int order)
{
    ...
    list_for_each_entry_rcu(range, &d->arch.hvm.pinned_cacheattr_ranges, list)
    {
                                 ...
                                 uint64_t gfn_end, uint32_t type)
{
    ...
    case XEN_DOMCTL_DELETE_MEM_CACHEATTR:
        ...
        domain_lock(d);
        list_for_each_entry(range, &d->arch.hvm.pinned_cacheattr_ranges, list)
...
static int cf_check hvm_save_mtrr_msr(struct vcpu *v, hvm_domain_context_t *h)
{
    ...
    struct hvm_hw_mtrr hw_mtrr = {
        .msr_mtrr_def_type = m->def_type |
                             MASK_INSR(m->fixed_enabled, MTRRdefType_FE) |
    ...
    BUILD_BUG_ON(sizeof(hw_mtrr.msr_mtrr_fixed) != sizeof(m->fixed_ranges));

    memcpy(hw_mtrr.msr_mtrr_fixed,
           m->fixed_ranges,
