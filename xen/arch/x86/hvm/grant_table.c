...
int create_grant_p2m_mapping(uint64_t addr, mfn_t frame, unsigned int flags,
                             unsigned int cache_flags)
{
    ...
    rc = p2m_add_page(current->domain,
...
int replace_grant_p2m_mapping(uint64_t addr, mfn_t frame, uint64_t new_addr,
                              unsigned int flags)
{
    ...
    if ( !p2m_is_grant(type) || !mfn_eq(old_mfn, frame) )
    {
        ...
        gdprintk(XENLOG_WARNING,
                 "old mapping invalid (type %d, mfn %" PRI_mfn
                 ", frame %" PRI_mfn ")\n",
                 type,
                 mfn_x(old_mfn),
                 mfn_x(frame));
