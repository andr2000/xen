...
static int cf_check ppro_check_ctrs(const unsigned int cpu,
...
out:
    printk(
        XENLOG_G_WARNING
        "Insufficient memory for oprofile," " oprofile is unavailable on dom%d vcpu%d\n",
        v->vcpu_id,
        v->domain->domain_id);
    return 0;
}
...
static void cf_check ppro_load_msr(struct vcpu *v, int type, int index,
                                   u64 *msr_content)
{
...
static void cf_check ppro_save_msr(struct vcpu *v, int type, int index,
                                   u64 msr_content)
{
    struct arch_msr_pair *msrs = vcpu_vpmu(v)->context;
