...
static int p4_unused_cccr[NUM_UNUSED_CCCRS] = {
    MSR_P4_BPU_CCCR1,   MSR_P4_BPU_CCCR3,   MSR_P4_MS_CCCR1, MSR_P4_MS_CCCR3,
    MSR_P4_FLAME_CCCR1, MSR_P4_FLAME_CCCR3, MSR_P4_IQ_CCCR0, MSR_P4_IQ_CCCR1,
...
static const struct p4_event_binding p4_events[NUM_EVENTS] = {
    ...
    { /* BRANCH_RETIRED */
      0x05, 0x06,
     { { CTR_IQ_4, MSR_P4_CRU_ESCR2 }, { CTR_IQ_5, MSR_P4_CRU_ESCR3 } }     },

    { /* MISPRED_BRANCH_RETIRED */
      0x04, 0x03,
     { { CTR_IQ_4, MSR_P4_CRU_ESCR0 }, { CTR_IQ_5, MSR_P4_CRU_ESCR1 } }     },

    { /* TC_DELIVER_MODE */
      0x01, 0x01,
     { { CTR_MS_0, MSR_P4_TC_ESCR0 }, { CTR_MS_2, MSR_P4_TC_ESCR1 } }       },

    { /* BPU_FETCH_REQUEST */
      0x00, 0x03,
     { { CTR_BPU_0, MSR_P4_BPU_ESCR0 }, { CTR_BPU_2, MSR_P4_BPU_ESCR1 } }   },

    { /* ITLB_REFERENCE */
      0x03, 0x18,
     { { CTR_BPU_0, MSR_P4_ITLB_ESCR0 }, { CTR_BPU_2, MSR_P4_ITLB_ESCR1 } } },
    ...
    { /* MOB_LOAD_REPLAY */
      0x02, 0x03,
     { { CTR_BPU_0, MSR_P4_MOB_ESCR0 }, { CTR_BPU_2, MSR_P4_MOB_ESCR1 } }   },

    { /* PAGE_WALK_TYPE */
      0x04, 0x01,
     { { CTR_BPU_0, MSR_P4_PMH_ESCR0 }, { CTR_BPU_2, MSR_P4_PMH_ESCR1 } }   },

    { /* BSQ_CACHE_REFERENCE */
      0x07, 0x0c,
     { { CTR_BPU_0, MSR_P4_BSU_ESCR0 }, { CTR_BPU_2, MSR_P4_BSU_ESCR1 } }   },

    { /* IOQ_ALLOCATION */
      0x06, 0x03,
     { { CTR_BPU_0, MSR_P4_FSB_ESCR0 }, { 0, 0 } }                          },

    { /* IOQ_ACTIVE_ENTRIES */
      0x06, 0x1a,
     { { CTR_BPU_2, MSR_P4_FSB_ESCR1 }, { 0, 0 } }                          },

    { /* FSB_DATA_ACTIVITY */
      0x06, 0x17,
     { { CTR_BPU_0, MSR_P4_FSB_ESCR0 }, { CTR_BPU_2, MSR_P4_FSB_ESCR1 } }   },

    { /* BSQ_ALLOCATION */
      0x07, 0x05,
     { { CTR_BPU_0, MSR_P4_BSU_ESCR0 }, { 0, 0 } }                          },

    { /* BSQ_ACTIVE_ENTRIES */
      0x07, 0x06,
     { { CTR_BPU_2, MSR_P4_BSU_ESCR1 /* guess */ }, { 0, 0 } }              },
    ...
    { /* TC_MS_XFER */
      0x00, 0x05,
     { { CTR_MS_0, MSR_P4_MS_ESCR0 }, { CTR_MS_2, MSR_P4_MS_ESCR1 } }       },
    ...
    { /* UOP_TYPE */
      0x02, 0x02,
     { { CTR_IQ_4, MSR_P4_RAT_ESCR0 }, { CTR_IQ_5, MSR_P4_RAT_ESCR1 } }     },

    { /* RETIRED_MISPRED_BRANCH_TYPE */
      0x02, 0x05,
     { { CTR_MS_0, MSR_P4_TBPU_ESCR0 }, { CTR_MS_2, MSR_P4_TBPU_ESCR1 } }   },
...
static void cf_check p4_fill_in_addresses(struct op_msrs *const msrs)
{
    ...
    {
        msrs->counters[i].addr = p4_counters[VIRT_CTR(stag, i)].counter_address;
    }

    /* FIXME: bad feeling, we don't save the 10 counters we don't use. */

    /* 18 CCCR registers */
    for ( i = 0, addr = MSR_P4_BPU_CCCR0 + stag; addr <= MSR_P4_IQ_CCCR5;
    ...
    for ( addr = MSR_P4_BSU_ESCR0 + stag; addr < MSR_P4_IQ_ESCR0;
          ++i, addr += addr_increment() )
    {
        msrs->controls[i].addr = addr;
    }
    ...
    {
        for ( addr = MSR_P4_BSU_ESCR0 + stag; addr <= MSR_P4_BSU_ESCR1;
              ++i, addr += addr_increment() )
        {
            msrs->controls[i].addr = addr;
        }
    ...
    {
        for ( addr = MSR_P4_IQ_ESCR0 + stag; addr <= MSR_P4_IQ_ESCR1;
              ++i, addr += addr_increment() )
        {
            msrs->controls[i].addr = addr;
        }
    ...
    for ( addr = MSR_P4_RAT_ESCR0 + stag; addr <= MSR_P4_SSU_ESCR0;
          ++i, addr += addr_increment() )
    {
        msrs->controls[i].addr = addr;
    }

    for ( addr = MSR_P4_MS_ESCR0 + stag; addr <= MSR_P4_TC_ESCR1;
          ++i, addr += addr_increment() )
    {
        msrs->controls[i].addr = addr;
    }

    for ( addr = MSR_P4_IX_ESCR0 + stag; addr <= MSR_P4_CRU_ESCR3;
          ++i, addr += addr_increment() )
    {
        msrs->controls[i].addr = addr;
    }
...
static void pmc_setup_one_p4_counter(unsigned int ctr)
{
    ...
    if ( counter_config[ctr].event <= 0 ||
         counter_config[ctr].event > NUM_EVENTS )
    {
    ...
    printk(KERN_ERR "oprofile: P4 event code %#lx no binding, stag %d ctr %d\n",
           counter_config[ctr].event,
           stag,
           ctr);
}
...
static int cf_check p4_check_ctrs(const unsigned int cpu,
