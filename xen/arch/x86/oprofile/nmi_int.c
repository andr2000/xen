...
static int passive_domain_msr_op_checks(unsigned int msr, int *typep,
                                        int *indexp)
{
...
{
    const unsigned int nr_ctrs = model->num_counters;
    const unsigned int nr_ctrls = model->num_controls;
...
static int __init p4_init(const char **cpu_type)
{
    ...
    {
        printk(
            "xenoprof: Initialization failed. " "Intel processor model %u for pentium 4 family is not " "supported\n",
            cpu_model);
        return 0;
    }
...
static int __init cf_check nmi_init(void)
{
    ...
    case X86_VENDOR_AMD:
        ...
        default:
            printk(
                "xenoprof: Initialization failed. " "AMD processor family %u is not " "supported\n",
                family);
            return -ENODEV;
        case 0xf:
    ...
    case X86_VENDOR_INTEL:
        ...
        {
            printk(
                "xenoprof: Initialization failed. " "Intel processor family %u model %d is not supported\n",
                family,
    ...
    default:
        printk(
            "xenoprof: Initialization failed. " "Unsupported processor. Unknown vendor %u\n",
            vendor);
        return -ENODEV;
    }
