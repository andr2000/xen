...
static void __init get_fam10h_pci_mmconf_base(void)
{
    ...
    {
        val = pci_conf_read32(PCI_SBDF(0, bus, slot, 1), 0x80 + (i << 3));
        if ( !(val & 3) )
            continue;

        start = (val & 0xffffff00) << 8; /* 39:16 on 31:8*/
        val = pci_conf_read32(PCI_SBDF(0, bus, slot, 1), 0x84 + (i << 3));
...
static const struct dmi_system_id __initconstrel mmconf_dmi_table[] = {
    {
     ...
     DMI_MATCH1(DMI_MATCH(DMI_SYS_VENDOR, "Sun Microsystems")),
     },
    {}
};
