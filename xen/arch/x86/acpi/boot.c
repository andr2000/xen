...
acpi_parse_x2apic(struct acpi_subtable_header *header, const unsigned long end)
{
    ...
    if ( madt_revision >= 5 && !(processor->lapic_flags & ACPI_MADT_ENABLED) &&
         ...
         processor->uid >= MAX_MADT_ENTRIES )
    {
        if ( log )
            printk(
                "%sAPIC ID %#x and/or ACPI ID %#x beyond limit" " - processor ignored\n",
...
static int __init cf_check acpi_parse_lapic(struct acpi_subtable_header *header,
...
static int __init cf_check acpi_parse_lapic_addr_ovr(
...
static int __init cf_check acpi_parse_x2apic_nmi(
    struct acpi_subtable_header *header, const unsigned long end)
{
...
static int __init cf_check acpi_parse_lapic_nmi(
    struct acpi_subtable_header *header, const unsigned long end)
{
...
static int __init cf_check acpi_parse_int_src_ovr(
    struct acpi_subtable_header *header, const unsigned long end)
{
...
acpi_fadt_parse_sleep_info(const struct acpi_table_fadt *fadt)
{
         ...
         fadt->header.length >= ACPI_FADT_V5_SIZE )
    {
                   ...
                   fadt->sleep_status.access_width * 8)) )
        {
            printk(
                KERN_WARNING PREFIX
                "Invalid sleep control/status register data:" " %#x:%#x:%#x %#x:%#x:%#x\n",
                fadt->sleep_control.bit_offset,
                fadt->sleep_control.bit_width,
    ...
    if ( fadt->flags & ACPI_FADT_HW_REDUCED )
    {
        ...
        memset(&acpi_sinfo.sleep_status + 1,
               0,
    ...
    {
        printk(KERN_WARNING PREFIX
               "32/64X FACS address mismatch in FADT - " "%08x/%016" PRIx64
               ", using 32\n",
               fadt->facs,
               facs_pa);
    ...
    if ( facs_pa % 64 )
        printk(KERN_WARNING PREFIX "FACS is not 64-byte aligned: %#lx\n",
               facs_pa);

    acpi_sinfo.wakeup_vector = facs_pa + offsetof(struct acpi_table_facs,
...
static int __init cf_check acpi_parse_fadt(struct acpi_table_header *table)
{
         ...
         fadt->xpm_timer_block.space_id == ACPI_ADR_SPACE_SYSTEM_IO )
    {
        ...
        if ( fadt->xpm_timer_block.access_width != 0 &&
            ...
            printk(KERN_WARNING PREFIX
                   "PM-Timer has invalid access width(%u)\n",
