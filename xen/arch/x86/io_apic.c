...
static void share_vector_maps(unsigned int src, unsigned int dst)
{
    ...
    bitmap_or(vector_map[src]->_bits,
              vector_map[src]->_bits,
              vector_map[dst]->_bits,
              X86_NR_VECTORS);
 ...
 */
static void __init replace_pin_at_irq(unsigned int irq, int oldapic, int oldpin,
...
struct IO_APIC_route_entry **alloc_ioapic_entries(void)
{
    ...
    {
        ioapic_entries[apic] = xmalloc_array(struct IO_APIC_route_entry,
...
struct IO_APIC_route_entry __ioapic_read_entry(unsigned int apic,
                                               unsigned int pin, bool raw)
{
...
static struct IO_APIC_route_entry ioapic_read_entry(unsigned int apic,
                                                    unsigned int pin, bool raw)
{
    struct IO_APIC_route_entry entry;
    unsigned long flags;
...
static void ioapic_write_entry(unsigned int apic, unsigned int pin, bool raw,
                               struct IO_APIC_route_entry e)
{
    unsigned long flags;
    spin_lock_irqsave(&ioapic_lock, flags);
 ...
 */
int restore_IO_APIC_setup(struct IO_APIC_route_entry **ioapic_entries, bool raw)
{
    int apic, pin;
...
static void cf_check set_ioapic_affinity_irq(struct irq_desc *desc,
...
static int EISA_ELCR(unsigned int irq)
{
    ...
    apic_printk(APIC_VERBOSE,
                KERN_INFO "Broken MPtable reports ISA irq %d\n",
                irq);
    return 0;
}
    ...
    for ( apic = 0; apic < nr_ioapics; apic++ )
    {
        for ( pin = 0; pin < nr_ioapic_entries[apic]; pin++ )
        {
            ...
            if ( idx == -1 )
            {
                ...
                {
                    apic_printk(APIC_VERBOSE,
                                KERN_DEBUG " IO-APIC (apicid-pin) %d-%d",
...
static void /*__init*/ __print_IO_APIC(bool boot)
{
    ...
    for ( apic = 0; apic < nr_ioapics; apic++ )
    {
        ...
        if ( (reg_01.bits.entries != 0x0f) && /* older (Neptune) boards */
             ...
             (reg_01.bits.entries != 0x2E) && (reg_01.bits.entries != 0x3F) )
            UNEXPECTED_IO_APIC();
...
void __init enable_IO_APIC(void)
{
    ...
    for ( apic = 0; apic < nr_ioapics; apic++ )
    {
        ...
        {
            struct IO_APIC_route_entry entry =
                ioapic_read_entry(apic, pin, false);
    ...
    if ( ((ioapic_i8259.apic != i8259_apic) ||
          (ioapic_i8259.pin != i8259_pin)) &&
...
static void __init setup_ioapic_ids_from_mpc(void)
{
    ...
    for ( apic = 0; apic < nr_ioapics; apic++ )
    {
        ...
        {
            apic_printk(APIC_VERBOSE,
                        "Setting %d in the " "phys_id_present_map\n",
                        mp_ioapics[apic].mpc_apicid);
        }
         ...
         */
        apic_printk(APIC_VERBOSE,
                    KERN_INFO "...changing IO-APIC physical APIC ID to %d ...",
                    mp_ioapics[apic].mpc_apicid);
...
static void __init check_timer(void)
{
    ...
    if ( pin1 != -1 )
    {
             ...
             hpet_enable_legacy_replacement_mode() )
        {
            printk(
                XENLOG_ERR
                "..no 8254 timer found - trying HPET Legacy Replacement Mode\n");
            ...
            printk(
                XENLOG_ERR
                "..no HPET timer found - reverting Legacy Replacement Mode\n");
            hpet_disable_legacy_replacement_mode();
        }
...
int __init io_apic_get_unique_id(int ioapic, int apic_id)
{
    ...
    {
        printk(KERN_WARNING "IOAPIC[%d]: Invalid apic_id %d, trying " "%d\n",
               ioapic,
               apic_id,
               reg_00.bits.ID);
    ...
    if ( physid_isset(apic_id, apic_id_map) )
    {
        ...
        printk(KERN_WARNING
               "IOAPIC[%d]: apic_id %d already used, " "trying %d\n",
               ioapic,
               apic_id,
               i);
    ...
    apic_printk(APIC_VERBOSE,
                KERN_INFO "IOAPIC[%d]: Assigned apic_id %d\n",
                ioapic,
                apic_id);
                            ...
                            int active_high_low)
{
    ...
    apic_printk(
        APIC_DEBUG,
        KERN_DEBUG
        "IOAPIC[%d]: Set PCI routing entry " "(%d-%d -> %#x -> IRQ %d Mode:%i Active:%i)\n",
        ioapic,
...
static int apic_pin_2_gsi_irq(int apic, int pin)
{
    ...
    return idx >= 0 ? pin_2_irq(idx, apic, pin) : io_apic_gsi_base(apic) + pin;
}
...
int ioapic_guest_write(unsigned long physbase, unsigned int reg, u32 val)
{
    ...
    *(u32 *)&rte = val;
    ...
    if ( pirq >= 0 )
    {
        ...
        ret =
            map_domain_pirq(hardware_domain, pirq, irq, MAP_PIRQ_TYPE_GSI, NULL);
    ...
    desc->handler = rte.trigger ? &ioapic_level_type : &ioapic_edge_type;
...
static const char *
delivery_mode_2_str(const enum ioapic_irq_destination_types mode)
{
    switch ( mode )
    {
    case dest_Fixed:
...
void dump_ioapic_irq_info(void)
{
    ...
    for ( irq = 0; irq < nr_irqs_gsi; irq++ )
    {
        ...
        for ( ;; )
        {
            ...
            printk(
                "vec=%02x delivery=%-5s dest=%c status=%d " "polarity=%d irr=%d trig=%c mask=%d dest_id:%0*x\n",
                rte.vector,
                delivery_mode_2_str(rte.delivery_mode),
                rte.dest_mode ? 'L' : 'P',
                rte.delivery_status,
                rte.polarity,
                rte.irr,
...
static __init bool bad_ioapic_register(unsigned int idx)
{
    ...
    {
        printk(KERN_WARNING
               "I/O APIC %#x registers return all ones, skipping!\n",
...
static void __init ioapic_init_mappings(void)
{
    ...
    for ( i = 0; i < nr_ioapics; i++, idx++ )
    {
        ...
        if ( rangeset_add_singleton(mmio_ro_ranges, ioapic_phys >> PAGE_SHIFT) )
...
void __init ioapic_init(void)
{
    ...
    else if ( nr_irqs != 0 && max_gsi_irqs > nr_irqs )
    {
        printk(
            XENLOG_WARNING
            "\"max_gsi_irqs=\" cannot be specified larger" " than \"nr_irqs=\"\n");
...
unsigned int __hwdom_init arch_hwdom_irqs(const struct domain *d)
{
    ...
    const unsigned int max_irqs =
        min_t(unsigned int, nr_irqs, PAGE_SIZE *BITS_PER_BYTE);
