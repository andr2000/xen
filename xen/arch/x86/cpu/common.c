...
bool __init probe_cpuid_faulting(void)
{
    ...
    if ( rc || !(val & MSR_PLATFORM_INFO_CPUID_FAULTING) ||
...
void ctxt_switch_levelling(const struct vcpu *next)
{
    ...
    if ( cpu_has_cpuid_faulting || boot_cpu_has(X86_FEATURE_CPUID_USER_DIS) )
    {
        /*
		 ...
		 */
        enable_cpuid_faulting =
            nextd &&
            (opt_dom0_cpuid_faulting || !is_control_domain(nextd) ||
             !is_pv_domain(nextd)) &&
            (is_pv_domain(nextd) ||
...
void display_cacheinfo(struct cpuinfo_x86 *c)
{
    ...
    if ( c->extended_cpuid_level >= 0x80000005 )
    {
        ...
        if ( (edx | ecx) >> 24 )
        {
            if ( opt_cpu_info )
                printk(
                    "CPU: L1 I cache %uK (%u bytes/line)," " D cache %uK (%u bytes/line)\n",
                    edx >> 24,
                    edx & 0xFF,
                    ecx >> 24,
                    ecx & 0xFF);
...
void detect_ht(struct cpuinfo_x86 *c)
{
    ...
    if ( !cpu_has(c, X86_FEATURE_HTT) || cpu_has(c, X86_FEATURE_CMP_LEGACY) ||
...
unsigned int __init apicid_to_socket(unsigned int apicid)
{
    ...
    if ( boot_cpu_has(X86_FEATURE_XTOPOLOGY) )
    {
        ...
        do
        {
            cpuid_count(0xb, sub_index, &eax, &dummy, &ecx, &dummy);
         ...
         !boot_cpu_has(X86_FEATURE_CMP_LEGACY) )
    {
        ...
        if ( num_siblings )
            return _phys_pkg_id(apicid, get_count_order(num_siblings));
    }
...
void load_system_tables(void)
{
    ...
    if ( cpu_has_xen_shstk )
    {
        ...
        unsigned long mce_ssp = stack_top + (IST_MCE * IST_SHSTK_SIZE) - 8,
    ...
    _set_tssldt_desc(gdt + TSS_ENTRY,
                     (unsigned long)tss,
                     sizeof(*tss) - 1,
                     SYS_DESC_tss_avail);
    if ( IS_ENABLED(CONFIG_PV32) )
        _set_tssldt_desc(this_cpu(compat_gdt) - FIRST_RESERVED_GDT_ENTRY +
                             TSS_ENTRY,
                         (unsigned long)tss,
                         sizeof(*tss) - 1,
                         SYS_DESC_tss_busy);
