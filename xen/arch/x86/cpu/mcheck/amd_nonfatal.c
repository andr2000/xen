...
static void cf_check mce_amd_work_fn(void *data)
{
    ...
    if ( adjust > 0 )
    {
        if ( !dom0_vmce_enabled() )
        {
            ...
            printk(
                "MCE: polling routine found correctable error. " " Use mcelog to parse above error output.\n");
        }
    }
    ...
    if ( hw_threshold )
    {
        ...
        if ( counter > 0 )
        {
            ...
            {
                printk(
                    "CPU counter reports %" PRIu32
                    " correctable hardware error%s that %s" " not reported by the status MSRs\n",
                    counter,
                    (counter == 1 ? "" : "s"),
                    (counter == 1 ? "was" : "were"));
            }
...
void __init amd_nonfatal_mcheck_init(struct cpuinfo_x86 *c)
{
    ...
    if ( variable_period && cpu_has(c, X86_FEATURE_SVM) )
    {
        ...
        if ( hw_threshold )
        {
            ...
            printk(XENLOG_INFO
                   "MCA: Use hw thresholding to adjust polling frequency\n");
        }
    }
