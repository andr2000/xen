...
void mctelem_defer(mctelem_cookie_t cookie, bool lmce)
{
    ...
    else
    {
		 ...
		 */
        if ( ctl->lmce_pending )
            mctelem_xchg_head(&ctl->lmce_pending, &ctl->pending, NULL);
        mctelem_xchg_head(&ctl->pending, &tep->mcte_next, tep);
    }
}
...
void mctelem_process_deferred(unsigned int cpu,
                              int (*fn)(mctelem_cookie_t mctc), bool lmce)
{
...
{
    int which = MCTE_CLASS(tep) == MCTE_F_CLASS_URGENT ? MC_URGENT
                                                       : MC_NONURGENT;
...
void __init mctelem_init(unsigned int datasz)
{
    ...
    if ( (mctctl.mctc_elems = xmalloc_array(struct mctelem_ent, MC_NENT)) ==
             NULL ||
...
{
    mctelem_class_t target = which == MC_URGENT ? MC_URGENT : MC_NONURGENT;
