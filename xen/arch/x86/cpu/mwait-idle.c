...
static const struct x86_cpu_id intel_idle_ids[]
    __initconstrel = { ICPU(NEHALEM_EP, nehalem),
...
static int __init mwait_idle_probe(void)
{
    ...
    if ( !(ecx & CPUID5_ECX_EXTENSIONS_SUPPORTED) ||
         !(ecx & CPUID5_ECX_INTERRUPT_BREAK) || !mwait_substates )
        return -ENODEV;
...
static int cf_check mwait_idle_cpu_init(struct notifier_block *nfb,
                                        unsigned long action, void *hcpu)
{
    ...
    for ( cstate = 0; cpuidle_state_table[cstate].target_residency; ++cstate )
    {
        ...
        num_substates = (mwait_substates >> (state * 4)) & MWAIT_SUBSTATE_MASK;
        /* If NO sub-states for this state in CPUID, skip it. */
        if ( num_substates == 0 )
            continue;

        /* if state marked as disabled, skip it */
        if ( cpuidle_state_table[cstate].flags & CPUIDLE_FLAG_DISABLED )
        {
...
bool __init mwait_pc10_supported(void)
{
    ...
    if ( boot_cpu_data.x86_vendor != X86_VENDOR_INTEL || !cpu_has_monitor ||
    ...
    return (ecx & CPUID5_ECX_EXTENSIONS_SUPPORTED) &&
           (ecx & CPUID5_ECX_INTERRUPT_BREAK) && (edx >> 28);
}
