                                 ...
                                 unsigned int hi)
{
    ...
    asm volatile(
        "1: wrmsr\n2:\n" ".section .fixup,\"ax\"\n" "3: movl %6,%0\n" "   jmp 2b\n" ".previous\n" _ASM_EXTABLE(
            ...
            3b)
        : "=r"(err)
        : "c"(msr), "a"(lo), "d"(hi), "D"(0x9c5a203a), "0"(0), "i"(-EFAULT));
...
#define CAN_K8(rev)       CAN(0x0f,    K8,          rev)
    CAN_FAM(11, B), CAN_FAM(10, C), CAN_FAM(10, B), CAN_K8(G),
    CAN_K8(F),      CAN_K8(E),      CAN_K8(D),      CAN_K8(C)
#undef CAN
};
...
static void __init noinline probe_masking_msrs(void)
{
    ...
    cpuidmask_defaults._1cd = _probe_mask_msr(MSR_K8_FEATURE_MASK, LCAP_1cd);
    cpuidmask_defaults.e1cd = _probe_mask_msr(MSR_K8_EXT_FEATURE_MASK,
                                              LCAP_e1cd);

    if ( c->cpuid_level >= 7 )
        cpuidmask_defaults._7ab0 = _probe_mask_msr(MSR_AMD_L7S0_FEATURE_MASK,
                                                   LCAP_7ab0);

    if ( c->x86 == 0x15 && c->cpuid_level >= 6 && cpuid_ecx(6) )
        cpuidmask_defaults._6c = _probe_mask_msr(MSR_AMD_THRM_FEATURE_MASK,
                                                 LCAP_6c);
	 ...
	 */
    if ( (expected_levelling_cap == levelling_caps) || cpu_has_hypervisor )
        return;

    printk(
        XENLOG_WARNING
        "Mismatch between expected (%#x) " "and real (%#x) levelling caps: missing %#x\n",
    ...
    printk(XENLOG_WARNING "Fam %#x, model %#x level %#x\n",
           ...
           c->x86_model,
           c->cpuid_level);
    printk(XENLOG_WARNING "If not running virtualised, please report a bug\n");
}
...
static void cf_check amd_ctxt_switch_masking(const struct vcpu *next)
{
    ...
    const struct cpuidmasks *masks = (nextd && is_pv_domain(nextd) &&
                                      nextd->arch.pv.cpuidmasks)
                                         ? nextd->arch.pv.cpuidmasks
                                         : &cpuidmask_defaults;
...
static const typeof(ctxt_switch_masking) __initconst_cf_clobber
__used csm = amd_ctxt_switch_masking;
#endif
...
static void __init noinline amd_init_levelling(void)
{
    ...
    if ( opt_cpu_info )
    {
        ...
        printk(
            XENLOG_INFO
            "MSR defaults: 1d 0x%08x, 1c 0x%08x, e1d 0x%08x, " "e1c 0x%08x, 7a0 0x%08x, 7b0 0x%08x, 6c 0x%08x\n",
...
int cpu_has_amd_erratum(const struct cpuinfo_x86 *cpu, int osvw_id, ...)
{
    ...
    if ( osvw_id >= 0 && cpu_has(cpu, X86_FEATURE_OSVW) )
    {
        ...
        if ( osvw_id < osvw_len )
        {
            ...
            rdmsrl(MSR_AMD_OSVW_STATUS + (osvw_id >> 6), osvw_bits);
...
static void disable_c1_ramping(void)
{
    ...
    nr_nodes = ((pci_conf_read32(PCI_SBDF(0, 0, 0x18, 0), 0x60) >> 4) & 0x07) +
               1;
...
static void check_syscfg_dram_mod_en(void)
{
    ...
    printk_once(KERN_ERR
                "MTRR: SYSCFG[MtrrFixDramModEn] found set; clearing\n");
...
static void amd_get_topology(struct cpuinfo_x86 *c)
{
    ...
    if ( opt_cpu_info )
        printk("CPU %d(%d) -> Processor %d, %s %d\n",
               ...
               c->x86_max_cores,
               c->phys_proc_id,
               c->compute_unit_id != INVALID_CUID ? "Compute Unit" : "Core",
    ...
    (0x80000000 | ((sbdf).bdf << 8) | ((reg) & 0xfc) | (((reg) & 0xf00) << 16))
        ...
        for ( ;; )
        {
            ...
            default:
                ...
                idx &= pci_conf_read(PCI_ECS_ADDRESS(sbdf, 0x15c), 0, 4) >> 2;
                break;
            }
            break;
        }
...
#define FREQ(v) (c->x86 < 0x17 ? ((((v) & 0x3f) + 0x10) * 100) >> (((v) >> 6) & 7) \
    ...
    if ( idx && idx < h && !rdmsr_safe(0xC0010064 + idx, val) && (val >> 63) &&
...
static void core_set_legacy_ssbd(bool enable)
{
    ...
    status = &ssbd_ls_cfg[c->phys_proc_id * ssbd_max_cores + c->cpu_core_id];
...
static void cf_check init_amd(struct cpuinfo_x86 *c)
{
    ...
    case 0x16:
		 ...
		 */
        if ( c == &boot_cpu_data && cpu_has(c, X86_FEATURE_RDRAND) &&
    ...
    if ( (c->x86 == 0x15) && (c->x86_model >= 0x10) && (c->x86_model <= 0x1f) &&
         !cpu_has(c, X86_FEATURE_TOPOEXT) &&
    ...
    if ( !cpu_has_amd_erratum(c, AMD_ERRATUM_121) )
        ...
        panic(
            "Xen will not boot on this CPU for security reasons" "Pass \"allow_unsafe\" if you're trusting all your" " (PV) guest kernels.\n");
    else if ( !opt_allow_unsafe && c == &boot_cpu_data )
        printk(
            KERN_WARNING
            "*** Xen will not allow creation of DomU-s on" " this CPU for security reasons. ***\n" KERN_WARNING
    ...
    if ( c->x86 == 0x16 && c->x86_model <= 0xf )
    {
        if ( c == &boot_cpu_data )
        {
            ...
            if ( l & 0x1f )
                pci_conf_write32(PCI_SBDF(0, 0, 0x18, 3), 0x58, l & ~0x1f);

            if ( h & 0x1 )
                pci_conf_write32(PCI_SBDF(0, 0, 0x18, 3), 0x5c, h & ~0x1);
        }
