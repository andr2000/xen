...
static void __init probe_masking_msrs(void)
{
    ...
    printk(XENLOG_WARNING
           "Fam %#x, model %#x expected (%#x/%#x/%#x), " "got (%#x/%#x/%#x)\n",
           c->x86,
           c->x86_model,
           exp_msr_basic,
           exp_msr_ext,
...
__used csm = intel_ctxt_switch_masking;
#endif
...
static void __init noinline intel_init_levelling(void)
{
    ...
    if ( opt_cpu_info )
    {
        ...
        if ( !cpu_has_cpuid_faulting )
            printk(
                XENLOG_INFO
                "MSR defaults: 1d 0x%08x, 1c 0x%08x, e1d 0x%08x, " "e1c 0x%08x, Da1 0x%08x\n",
...
static void intel_log_freq(const struct cpuinfo_x86 *c)
{
    ...
    switch ( c->x86 )
    {
        static const unsigned short core_factors[] = { 26667, 13333, 20000,
                                                       16667, 33333, 10000,
                                                       40000 };
...
static void cf_check init_intel(struct cpuinfo_x86 *c)
{
    ...
    if ( opt_arat && (c->cpuid_level >= 0x00000006) &&
