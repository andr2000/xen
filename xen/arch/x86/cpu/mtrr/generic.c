...
static const char *__init mtrr_attrib_to_str(mtrr_type x)
{
    static const char __initconst strings[MTRR_NUM_TYPES][16] = {
        [X86_MT_UC] = "uncachable",    [X86_MT_WC] = "write-combining",
        [X86_MT_WT] = "write-through", [X86_MT_WP] = "write-protect",
...
static void mtrr_wrmsr(unsigned int msr, uint64_t msr_content)
{
    if ( wrmsr_safe(msr, msr_content) < 0 )
        printk(KERN_ERR "MTRR: CPU %u: Writing MSR %x to %" PRIx64 " failed\n",
               smp_processor_id(),
               msr,
               msr_content);
...
int mtrr_get_free_region(unsigned long base, unsigned long size,
                         int replace_reg)
...
void mtrr_get(unsigned int reg, unsigned long *base, unsigned long *size,
              mtrr_type *type)
{
...
void mtrr_set(unsigned int reg, unsigned long base, unsigned long size,
              mtrr_type type)
...
int mtrr_validate_add_page(unsigned long base, unsigned long size,
                           unsigned int type)
{
