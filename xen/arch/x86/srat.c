...
{
    return ((pxm2node[idx].pxm == pxm) && (pxm2node[idx].node != NUMA_NO_NODE));
}
...
acpi_numa_memory_affinity_init(const struct acpi_srat_mem_affinity *ma)
{
    ...
    if ( !numa_update_node_memblks(node,
                                   pxm,
                                   ma->base_address,
                                   ma->length,
...
srat_parse_region(struct acpi_subtable_header *header, const unsigned long end)
{
    ...
    if ( !ma->length || !(ma->flags & ACPI_SRAT_MEM_ENABLED) ||
