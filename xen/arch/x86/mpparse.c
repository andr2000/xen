...
void __init set_nr_cpu_ids(unsigned int max_cpus)
{
    ...
    printk(XENLOG_INFO "SMP: Allowing %u CPUs (%d hotplug CPUs)\n",
           max_cpus,
           max_t(int, max_cpus - num_processors, 0));
...
void __init set_nr_sockets(void)
{
    nr_sockets = last_physid(phys_cpu_present_map) /
                 ...
                 1;
    if ( disabled_cpus )
        nr_sockets += (disabled_cpus - 1) / boot_cpu_data.x86_max_cores /
                          boot_cpu_data.x86_num_siblings +
                      1;
...
static int MP_processor_info_x(struct mpc_config_processor *m, u32 apicid,
                               bool hotplug)
{
    int ver, cpu = 0;
    ...
    if ( num_processors >= 8 && hotplug && genapic.name == apic_default.name )
    {
        printk_once(
            XENLOG_WARNING
    ...
    if ( !(m->mpc_cpuflag & CPU_BOOTPROCESSOR) )
    {
        ...
        if ( cpu < 0 )
        {
            printk(
                KERN_WARNING
                "WARNING: Can't alloc cpu_id." " Processor with apicid %i ignored\n",
                apicid);
            return cpu;
        }
...
static void __init MP_ioapic_info(struct mpc_config_ioapic *m)
{
    ...
    if ( !m->mpc_apicaddr )
    {
        printk(
            KERN_ERR
            "WARNING: bogus zero I/O APIC address" " found in MP table, skipping!\n");
        return;
    }
...
static void __init MP_lintsrc_info(struct mpc_config_lintsrc *m)
{
	 ...
	 */
    if ( (m->mpc_irqtype == mp_ExtINT) && (m->mpc_destapiclint != 0) )
        BUG();
    if ( (m->mpc_irqtype == mp_NMI) && (m->mpc_destapiclint != 1) )
        BUG();
}
    ...
    while ( count < mpc->mpc_length )
    {
        ...
        case MP_PROCESSOR:
        {
            ...
            printk("Processor #%02x %u:%u APIC version %u%s\n",
                   m->mpc_apicid,
                   MASK_EXTR(m->mpc_cpufeature, CPU_FAMILY_MASK),
                   MASK_EXTR(m->mpc_cpufeature, CPU_MODEL_MASK),
                   m->mpc_apicver,
                   m->mpc_cpuflag & CPU_ENABLED ? "" : " [disabled]");
            MP_processor_info(m);
            break;
        }
        case MP_BUS:
...
static inline void __init construct_default_ISA_mptable(int mpc_default_type)
{
    ...
    processor.mpc_featureflag = boot_cpu_data.x86_capability[FEATURESET_1d];
...
static int __init smp_scan_config(unsigned long base, unsigned long length)
{
    ...
    while ( length > 0 )
    {
        ...
        if ( (*bp == SMP_MAGIC_IDENT) && (mpf->mpf_length == 1) &&
...
static void __init efi_check_config(void)
{
    ...
    if ( memcmp(mpf->mpf_signature, "_MP_", 4) == 0 && mpf->mpf_length == 1 &&
...
void __init find_smp_config(void)
{
	 ...
	 */
    if ( smp_scan_config(0x0, 0x400) || smp_scan_config(639 * 0x400, 0x400) ||
...
int mp_register_lapic(u32 id, bool enabled, bool hotplug)
{
    struct mpc_config_processor processor = {
        ...
        .mpc_cpuflag = (enabled ? CPU_ENABLED : 0) |
                       (id == boot_cpu_physical_apicid ? CPU_BOOTPROCESSOR : 0),
    };
...
void __init mp_register_ioapic(u8 id, u32 address, u32 gsi_base)
{
    ...
    {
        printk(KERN_ERR
               "ERROR: Max # of I/O APICs (%d) exceeded " "(found %d)\n",
               MAX_IO_APICS,
               nr_ioapics);
        panic("Recompile kernel with bigger MAX_IO_APICS\n");
    }
    ...
    mp_ioapic_routing[idx].gsi_end = gsi_base + io_apic_get_redir_entries(idx);

    printk("IOAPIC[%d]: apic_id %d, version %d, address %#x, " "GSI %d-%d\n",
           idx,
...
void __init mp_override_legacy_irq(u8 bus_irq, u8 polarity, u8 trigger, u32 gsi)
{
    ...
    Dprintk("Int: type %d, pol %d, trig %d, bus %d, irq %d, %d-%d\n",
            intsrc.mpc_irqtype,
            intsrc.mpc_irqflag & 3,
            (intsrc.mpc_irqflag >> 2) & 3,
            intsrc.mpc_srcbus,
...
int mp_register_gsi(u32 gsi, int triggering, int polarity)
{
    ...
    {
        printk(KERN_ERR "Invalid reference to IOAPIC pin " "%d-%d\n",
    ...
    return io_apic_set_pci_routing(ioapic,
                                   ioapic_pin,
                                   gsi,
                                   triggering,
                                   polarity);
