...
{
    struct rcu_node *rcu_node = container_of(head, struct rcu_node, rcu_head);
    xfree(rcu_node);
}
...
static void radix_tree_node_free(struct radix_tree_root *root,
                                 struct radix_tree_node *node)
{
...
static unsigned int __lookup(struct radix_tree_node *slot, void ***results,
                             unsigned long index, unsigned int max_items,
                                    ...
                                    unsigned int max_items)
{
    ...
    while ( ret < max_items )
    {
        ...
        for ( i = 0; i < slots_found; i++ )
        {
            ...
            if ( !slot )
                continue;
            results[ret + nr_found] = indirect_to_ptr(rcu_dereference(slot));
            nr_found++;
        }
        ret += nr_found;
        if ( next_index == 0 )
...
static inline void radix_tree_shrink(struct radix_tree_root *root)
{
    ...
    while ( root->height > 0 )
    {
		 ...
		 */
        if ( root->height == 0 )
            *((unsigned long *)&to_free->slots[0]) |= RADIX_TREE_INDIRECT_PTR;
...
static void radix_tree_node_destroy(struct radix_tree_root *root,
                                    struct radix_tree_node *node,
                                    void (*slot_free)(void *))
{
    int i;
