...
/* Called twice */
static inline void __init rc_bit_tree_decode(struct rc *rc, uint16_t *p,
...
static inline uint8_t __init peek_old_byte(struct writer *wr, uint32_t offs)
{
    if ( !wr->flush )
    {
        int32_t pos;
...
static inline int __init copy_bytes(struct writer *wr, uint32_t rep0, int len)
{
    do
    {
                                      ...
                                      int pos_state, uint16_t *prob)
{
    ...
    else
    {
        ...
        if ( rc_is_bit_0(rc, prob_len) )
        {
            rc_update_bit_0(rc, prob_len);
            prob_len =
                (prob + LZMA_LEN_MID + (pos_state << LZMA_LEN_NUM_MID_BITS));
        ...
        else
        {
            ...
            offset =
                ((1 << LZMA_LEN_NUM_LOW_BITS) + (1 << LZMA_LEN_NUM_MID_BITS));
    ...
    if ( cst->state < 4 )
    {
        ...
        if ( ++(cst->rep0) == 0 )
            return 0;
        if ( cst->rep0 > wr->header->dict_size || cst->rep0 > get_pos(wr) )
            return -1;
    }
...
int __init unlzma(unsigned char *buf, unsigned int in_len,
                  int (*fill)(void *, unsigned int),
                  int (*flush)(void *, unsigned int), unsigned char *output,
                  unsigned int *posp, void (*error)(const char *x))
{
    struct lzma_header header;
    int lc, pb, lp;
