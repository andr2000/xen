...
static int __init get_next_block(struct bunzip_data *bd)
{
    ...
    int *base = NULL;
    int *limit = NULL;
    int dbufCount, nextSym, dbufSize, groupCount, selector, i, j, k, t, runPos,
        symCount, symTotal, nSelectors, byteCount[256];
...
int __init bunzip2(unsigned char *buf, unsigned int len,
                   int (*fill)(void *, unsigned int),
                   int (*flush)(void *, unsigned int), unsigned char *outbuf,
                   unsigned int *pos, void (*error)(const char *x))
{
    struct bunzip_data *bd;
    int i = -1;
    unsigned char *inbuf;
