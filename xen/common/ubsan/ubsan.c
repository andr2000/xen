                          ...
                          unsigned long value)
{
    if ( type_is_int(type) )
    {
        ...
        {
            scnprintf(str, size, "%lld", (long long)get_signed_val(type, value));
        }
        else
        {
            scnprintf(str,
                      size,
                      "%llu",
                      (unsigned long long)get_unsigned_val(type, value));
        }
    }
}
...
void __ubsan_handle_nonnull_arg(struct nonnull_arg_data *data)
{
    ...
    pr_err(
        "null pointer passed as argument %d, declared with nonnull attribute\n",
        data->arg_index);
...
void __noreturn __ubsan_handle_builtin_unreachable(struct unreachable_data *data)
{
    unsigned long flags;
