...
#define PREFIX_do_arm
                            ...
                            handle : uint unsigned int handle
    : const_void const void handle : const_char const char

#ifdef CONFIG_COMPAT
                                         defhandle : multicall_entry_compat_t
#ifndef CONFIG_PV_SHIM_EXCLUSIVE
                                                         defhandle
    : compat_platform_op_t
#endif
#endif
#ifdef CONFIG_PV32
          defhandle : trap_info_compat_t defhandle : physdev_op_compat_t
#endif

                                                         prefix
    : do PREFIX_hvm PREFIX_compat PREFIX_do_arm
      physdev_op(int cmd, void *arg)

          prefix
    : do PREFIX_hvm PREFIX_compat
#if defined(CONFIG_GRANT_TABLE) || defined(CONFIG_PV_SHIM)
      grant_table_op(unsigned int cmd, void *uop, unsigned int count)
#endif

          prefix : do PREFIX_hvm memory_op(unsigned long cmd, void *arg)

                       prefix
    : do PREFIX_compat xen_version(int cmd, void *arg)
          vcpu_op(int cmd, unsigned int vcpuid, void *arg)
              sched_op(int cmd, void *arg) xsm_op(void *op)
                  callback_op(int cmd, const void *arg)
#ifdef CONFIG_ARGO
                      argo_op(unsigned int cmd, void *arg1, void *arg2,
                              unsigned long arg3, unsigned long arg4)
#endif
#ifdef CONFIG_PV
                          iret() nmi_op(unsigned int cmd, void *arg)
...
#ifdef CONFIG_COMPAT
                                  prefix
    : compat set_timer_op(uint32_t lo, uint32_t hi) multicall(
          multicall_entry_compat_t *call_list,
          unsigned long nr_calls) memory_op(unsigned int cmd, void *arg)
#ifdef CONFIG_IOREQ_SERVER
          dm_op(domid_t domid, unsigned int nr_bufs, void *bufs)
#endif
              mmuext_op(void *arg, unsigned int count, uint *pdone,
                        unsigned int foreigndom)
#ifdef CONFIG_PV32
                  set_trap_table(trap_info_compat_t *traps) set_gdt(
                      unsigned int *frame_list, unsigned int entries)
                      set_callbacks(unsigned long event_selector,
                                    unsigned long event_address,
                                    unsigned long failsafe_selector,
                                    unsigned long failsafe_address)
                          update_descriptor(uint32_t pa_lo, uint32_t pa_hi,
                                            uint32_t desc_lo, uint32_t desc_hi)
                              update_va_mapping(unsigned int va, uint32_t lo,
                                                uint32_t hi, unsigned int flags)
                                  physdev_op_compat(physdev_op_compat_t *uop)
                                      update_va_mapping_otherdomain(
                                          unsigned int va, uint32_t lo,
                                          uint32_t hi, unsigned int flags,
                                          domid_t domid)
#endif
#ifndef CONFIG_PV_SHIM_EXCLUSIVE
                                          platform_op(
                                              compat_platform_op_t *u_xenpf_op)
#endif
#ifdef CONFIG_KEXEC
                                              kexec_op(unsigned int op,
                                                       void *uarg)
#endif
#endif /* CONFIG_COMPAT */
                                                  ...
                                                  prefix
    : do PREFIX_dep event_channel_op_compat(evtchn_op_t *uop)
      ...
      sched_op_compat(int cmd, unsigned long arg)
          ...
          prefix
    : do set_timer_op(s_time_t timeout) console_io(
          unsigned int cmd, unsigned int count,
          char *buffer) vm_assist(unsigned int cmd, unsigned int type)
          event_channel_op(int cmd, void *arg) mmuext_op(
              mmuext_op_t *uops, unsigned int count, unsigned int *pdone,
              unsigned int foreigndom) multicall(multicall_entry_t *call_list,
                                                 unsigned long nr_calls)
#ifdef CONFIG_PV
              mmu_update(mmu_update_t *ureqs, unsigned int count,
                         unsigned int *pdone,
                         unsigned int foreigndom) stack_switch(unsigned long ss,
                                                               unsigned long esp)
                  fpu_taskswitch(
                      int set) set_debugreg(int reg, unsigned long value)
                      get_debugreg(int reg) set_segment_base(
                          unsigned int which,
                          unsigned long base) mca(xen_mc_t *u_xen_mc)
                          set_trap_table(const_trap_info_t *traps) set_gdt(
                              xen_ulong_t
                                  *frame_list,
                              unsigned int
                                  entries) set_callbacks(unsigned long
                                                             event_address,
                                                         unsigned long
                                                             failsafe_address,
                                                         unsigned long
                              ...
                              update_descriptor(
                                  uint64_t gaddr,
                                  seg_desc_t desc) update_va_mapping(unsigned long
                                                                         va,
                                                                     uint64_t val64,
                                                                     unsigned long
                                                                         flags)
                                  update_va_mapping_otherdomain(
                                      unsigned long va, uint64_t val64,
...
#endif
#ifndef CONFIG_PV_SHIM_EXCLUSIVE
                                              sysctl(xen_sysctl_t *u_sysctl) domctl(
               ...
               caller : hvm64
                        ...
                        caller : hvm32
                                 ...
                                 caller : arm
                                          ...
                                          table
    : pv32 pv64 hvm32 hvm64 arm set_trap_table compat do -
      ...
      - -memory_op compat do hvm hvm do multicall compat:2 do:2 compat
        do do update_va_mapping compat do -
      - -set_timer_op compat do compat do -
      event_channel_op_compat do do -
      -dep xen_version do do do do do console_io
      do do do do do physdev_op_compat compat do -
      -dep
      ...
      - -
#endif
      vm_assist do do do do do update_va_mapping_otherdomain compat do -
      - -iret compat:1 do:1 -
      - -vcpu_op compat do compat:1 do:1 do set_segment_base do:2 do:2 -
      - -
#ifdef CONFIG_PV
        mmuext_op compat:2 do:2 compat do -
#endif
      xsm_op compat do compat do do nmi_op compat do -
      - -sched_op compat do compat do do callback_op compat do -
      - -
#ifdef CONFIG_XENOPROF
        xenoprof_op compat do -
      - -
#endif
        event_channel_op do do do:1 do:1 do:1 physdev_op compat
        do hvm hvm do_arm
#ifdef CONFIG_HVM
        hvm_op do do do do do
#endif
#ifndef CONFIG_PV_SHIM_EXCLUSIVE
        sysctl do do do do do domctl do do do do do
#endif
#ifdef CONFIG_KEXEC
        kexec_op compat do -
      - -
