    ...
    if ( enable )
    {
        ...
        for ( i = 0; i < PCI_SRIOV_NUM_BARS; i += rc )
        {
            ...
            const pci_sbdf_t sbdf =
                PCI_SBDF3(pdev->seg, pdev->bus, pdev->devfn);
...
{
    uint16_t offset = pci_conf_read16(pdev->sbdf,
                                      pos + PCI_SRIOV_VF_OFFSET);
    uint16_t stride = pci_conf_read16(pdev->sbdf,
    ...
    for ( i = 0; i < sriov->num_vfs; i++ )
    {
        ...
        if ( rc )
            gprintk(XENLOG_ERR, "failed to %s VF %pp: %d\n",
                    new_enabled ? "add" : "remove", &pdev->sbdf, rc);
...
static void enable_callback(void *data)
{
    ...
    enable_tail(pdev, cb->sriov, cb->pos, cb->new_enabled,
                cb->new_mem_enabled);
    pci_conf_write16(pdev->sbdf, cb->pos + PCI_SRIOV_CTRL,
                     cb->value);
    xfree(cb);
}
                          ...
                          uint32_t val, void *data)
{
    ...
    uint16_t control = pci_conf_read16(pdev->sbdf,
                                       pos + PCI_SRIOV_CTRL);
    ...
    if ( new_enabled != enabled )
    {
        if ( new_enabled )
        {
            ...
            struct vpci_vcpu_pending_task *wait_task =
                vpci_alloc_pending_task(pdev);
             ...
             */
            pci_conf_write16(pdev->sbdf, reg,
            ...
            pcidevs_lock();
            /*
             * NB: a non-const pci_dev of the PF is needed in order to
             * schedule a task.
             */
            wait_task->pdev = pci_get_pdev(pdev->seg, pdev->bus, pdev->devfn);
            pcidevs_unlock();
            ASSERT(wait_task->pdev);
            wait_task->wait.callback = enable_callback;
            wait_task->wait.data = cb;
            wait_task->wait.end = NOW() + 100 * cpu_khz;
            vpci_add_pending_task(wait_task);
            return;
        }
