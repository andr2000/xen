...
static struct vpci_msix_entry *get_entry(struct vpci_msix *msix,
                                         paddr_t addr)
{
    paddr_t start = vmsix_guest_table_addr(msix->pdev->vpci, VPCI_MSIX_TABLE);
...
int vpci_make_msix_hole(const struct pci_dev *pdev)
{
    ...
    if ( !is_hardware_domain(pdev->domain) )
    {
        ...
        {
            paddr_t addr = vmsix_guest_table_addr(pdev->vpci, VPCI_MSIX_TABLE);
            paddr_t size = vmsix_table_size(pdev->vpci, VPCI_MSIX_TABLE) - 1;
