              ...
              unsigned int *flush_flags)
{
    ...
    {
        rc = hd->platform_ops->map_pages(d, dfn, mfn, page_order, flags,
                                    flush_flags);
        if ( unlikely (rc) )
            if ( !d->is_shutting_down && printk_ratelimit() )
                printk(XENLOG_ERR
                       ...
                       dfn_x(dfn), mfn_x(mfn), page_order, rc);
    }
    else
    {
        unsigned long i;
                ...
                unsigned int *flush_flags)
{
    ...
    if (hd->platform_ops->map_pages)
    {
        rc = hd->platform_ops->unmap_pages(d, dfn, page_order, flush_flags);
        if ( unlikely (rc) )
            if ( !d->is_shutting_down && printk_ratelimit() )
                printk(XENLOG_ERR
                       "d%d: IOMMU unmapping dfn %"PRI_dfn" with order %"PRIu32
                       " failed: %d\n", d->domain_id, dfn_x(dfn),
                       page_order, rc);
    }
    else
    {
